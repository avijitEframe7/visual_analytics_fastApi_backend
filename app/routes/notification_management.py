...
def _incident_image_for_json(val: Any) -> str:
    """
    Incident_image is stored as BLOB (JPEG). Expose as base64 for JSON clients;
...
def get_notifications(db: Session = Depends(get_db)):
    ...
    try:
        result = db.execute(
            text("""
                SELECT
                    ...
                    el.incident_image AS Incident_image,
                    el.time_occurred,
                    el.updated_at,
                    el.camera_id,
                    c.camera_name,
        ...
        for row in rows:
            ...
            # If JOIN missed (deleted camera, legacy bad FK) but camera_id exists, resolve from camera config
