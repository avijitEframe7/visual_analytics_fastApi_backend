...
def _parse_stored_password(stored_raw) -> Tuple[Optional[str], str]:
    """
    Read password as stored in the database (hex string or binary).

    - BLOB/bytes length 32: raw SHA256 digest -> (64-char hex lower, "")
    - CHAR(64) hex from UNHEX/HEX style storage -> (hex lower, "")
...
def login(data: LoginRequest, db: Session = Depends(get_db)):
    ...
    # MySQL may return password column as str or bytes; handle both in _parse_stored_password.
...
def update_profile(
    ...
    """
    Update basic user profile details stored in `users`.
